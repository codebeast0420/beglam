...
const Edit = ({ navigation }) => {
  ...
  const name = useSelector((store) => store.userId.name);
  ...
  const getProfile = async () => {
    await fetch("http://95.216.115.245:8001/api/profile/getProfileByUser", {
      ...
      if (result.data) {
        setData(prev => ({ ...prev, 'description': result.data.description }));
        setData(prev => ({ ...prev, 'address': result.data.address }));
      }
    })
  }
  ...
  const save = async () => {
    ...
    //   return;
    // }
    await fetch("http://95.216.115.245:8001/api/profile/editProfile", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
